...
class CustomDataset(utils.Dataset):
    ...
    def load_custom(self, dataset_dir, subset):
        ...
        for a in annotations:
            ...
            self.add_image(
                "profile",  ## for a single class just add the name here
...
def mask(model, image_path=None):
    assert image_path
    ...
    # Color splash
    splash = img_as_uint(r['masks'])
    # Save output
    file = args.image
    file = file.split('/')[-1:]
    file_name = str(file) + "{:%Y%m%dT%H%M%S}.png".format(datetime.datetime.now())
...
def detect_and_color_splash(model, image_path=None, video_path=None):
    ...
    if image_path:
        ...
        while success:
            ...
            if success:
                # OpenCV returns data as BGR, convert to RGB
                image = image[..., ::-1]
                # Detect objects
...
if __name__ == '__main__':
    ...
    if args.command == "train":
        ...
        mask(model, image_path=args.image)
    else:
